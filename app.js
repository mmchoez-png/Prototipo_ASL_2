...
// Pronunciación de letras individuales vs. oraciones/palabras completas
function reproducirVoz(texto, esLecturaCompleta = false) {
  window.speechSynthesis.cancel();
  
  let textoAjustado = texto;

  if (esLecturaCompleta) {
    // Convierte a minúsculas para obligar al sintetizador a leer palabras reales y no letras sueltas
    textoAjustado = texto.toLowerCase();
  }

  const utterance = new SpeechSynthesisUtterance(textoAjustado);
  utterance.lang = 'en-US';
  utterance.rate = esLecturaCompleta ? 0.9 : 1.1; // Velocidad natural para palabras, un poco más rápida para letras
  window.speechSynthesis.speak(utterance);
}

// Botón "Read Sentence": Lee la palabra o frase completa fluida
function leerOracion() {
  if (sentenceText.trim()) {
    reproducirVoz(sentenceText, true);
  }
}

// Botón "Space": Opcional - Lee la última palabra completada al presionar espacio
function agregarEspacio() {
  if (sentenceText.length > 0 && !sentenceText.endsWith(" ")) {
    // Extrae la última palabra escrita
    const palabras = sentenceText.trim().split(" ");
    const ultimaPalabra = palabras[palabras.length - 1];
    
    if (ultimaPalabra) {
      reproducirVoz(ultimaPalabra, true); // Lee la palabra completa
    }
  }
